...
if __name__ == "__main__":
    try:
        print("get_pdeco_easy_loginテスト開始", flush=True)
        ...
        if not infos:
            ...
            for i, info in enumerate(infos):
                ...
                if len(info) >= 4:
                    cast_id = info[2]
                    cast_password = info[3]
                else:
                    print(f"⚠ キャスト情報が不完全です: {info}", flush=True)
                    skip_count += 1
                    continue
                ...
                try:
                    result = pdeco.get_pdeco_easy_login(cast_name, cast_id, cast_password, False)
            ...
            print(f"\n{'='*50}", flush=True)
            print("get_pdeco_easy_loginテスト結果サマリー", flush=True)
            print(f"{'='*50}", flush=True)
        ...
        print("\nget_pdeco_easy_loginテスト完了", flush=True)
